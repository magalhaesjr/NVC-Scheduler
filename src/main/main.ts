...
import { importTemplates, writeTemplate, readSheet } from './templates';
import { assignTeams, importTeamInfo } from './teams';
 ...
 *****                                                                   *****
 ***************************************************************************/
ipcMain.handle('scheduler:launchTeamPreview', (_, msg) => {
  // Pull out data from message
  const { teamPreview, teamNum } = msg;
  ...
  if (previewWin === null) {
    ...
    // Cleanup stuff
    previewWin.on('close', closePreview);
  } else {
    //just send an update
...
ipcMain.handle('scheduler:assignTeams', assignTeams);

// Load team info from excel csv
ipcMain.handle('scheduler:importTeamInfo', importTeamInfo);

// Load templates from stored file
ipcMain.handle('scheduler:importTemplates', () =>
  importTemplates(templateFile)
);

ipcMain.handle('scheduler:saveSchedule', saveSchedule);
/*****************************************************************************
 *****                                                                   *****
 *****                   Template Tool Thread Comms                      *****
 *****                                                                   *****
 ****************************************************************************/
ipcMain.handle('templateTool:writeTemplate', (_, template) =>
  writeTemplate(templateFile, template)
);
ipcMain.handle('templateTool:readSheet', readSheet);
